...
./deploy.sh staging               # Bump + deploy both to staging
...
Every deploy bumps the patch version in `package.json` + `apps/web/package.json`, commits, tags `vX.Y.Z`, and pushes after deploy.
