...
export class Trail {
  ...
  private rebuildGeometry(): void {
    ...
    const vertCount = (pts.length - 1) * 6;
    if (vertCount * 3 > this.maxVerts * 3) return;
    ...
    // Rebuild last TRAIL_RAMP_SEGMENTS + 1 segments to update ramp heights
    const totalSegs = pts.length - 1;
    const startSeg = Math.max(0, totalSegs - TRAIL_RAMP_SEGMENTS - 1);

    for (let i = startSeg; i < totalSegs; i++) {
      const p1 = pts[i];
      const p2 = pts[i + 1];
