...
export class Bike {
  ...
  /** Sync optional fields (invulnerability, double-jump, flight) from server state. */
    ...
    if (this.netBuffer.length >= 2 && renderTick !== undefined) {
      ...
      if (t >= 0 && t <= 1.0) {
        ...
        if (extraTicks <= EXTRAP_MAX_TICKS) {
          ...
          this.position.x = b.x + Math.sin(b.angle) * b.speed * cosPitch * cappedSec;
          this.position.z = b.z + Math.cos(b.angle) * b.speed * cosPitch * cappedSec;
          this.position.y = b.y;
          this.angle = b.angle;
        }
